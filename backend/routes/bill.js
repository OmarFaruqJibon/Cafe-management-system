...
router.post("/generate-report", auth.authenticate, (req, res) => {
                ...
                ejs.renderFile(path.join(__dirname, "", "report.ejs"),
                    ...
                    (err, results) => {
                        if (err) {
                            ...
                            pdf.create(results).toFile("./generated_bills/" + generatedUUID + ".pdf",
                                (err, data) => {
                                    if (err) {
                                        return res.status(500).json({ err });
                                    } else {
...
router.post("/get-pdf", auth.authenticate, (req, res) => {
    const orderDetails = req.body;
    const pdfPath = "./generated_bills/" + orderDetails.uuid + ".pdf";
        ...
        ejs.renderFile(
            ...
            (err, results) => {
                if (err) {
                    ...
                    pdf.create(results).toFile("./generated_bills/" + orderDetails.uuid + ".pdf",
                        (err, data) => {
                            if (err) {
                                console.log(err);
                                return res.status(500).json({ err });
...
router.get("/get-bills", auth.authenticate, (req, res, next) => {
